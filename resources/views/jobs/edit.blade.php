<x-layout>
  ...
  <div class="bg-white mx-auto p-8 rounded-lg shadow-md w-full md:max-w-3xl">
    ...
    <form method="POST" action="{{ route('jobs.update', $job->id) }}" enctype="multipart/form-data">
      ...
      <x-inputs.text-area id="description" name="description" label="Description" placeholder="We're seeking a skilled Software developer ..." :value="old('description', $job->description)" />    
      <x-inputs.text id="salary" name="salary" label="Salary" type="number" placeholder="90000" :value="old('salaray', $job->salaray)" />
      ...
      <x-inputs.text-area id="company_description" name="Company Description" label="company_description" placeholder="Company Description" :value="old('company_description', $job->company_description)" />    
      ...
      <x-inputs.text id="contact_email" name="contact_email" label="Email" type="email" placeholder="Enter contact email" :value="old('contact_email', $job->contact_email)" />
